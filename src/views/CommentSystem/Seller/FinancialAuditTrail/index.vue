	...
	<div>
		...
		<div class="tableMag">
			<el-table :data="tableData" border stripe v-loading="loading" style="width: 100%">
			    ...
			    <el-table-column prop="realmoney" label="实际金额(人民币)" min-width="6%" />
			    ...
			    <el-table-column fixed="right" label="操作" min-width="7%" >
			      <template slot-scope="scope">
						<el-switch v-if="scope.row.zt=='2'" v-model="scope.row.ifread" :active-value="1" :inactive-value="0" active-text="已读" inactive-text="未读" @change="cancel(scope.row)"></el-switch>
				  </template>
			    </el-table-column>
			  </el-table>
		</div>
...
<script>
	...
	export default({
		...
		created() {
			...
			if(query.type){
				this.fromorder.orderStaus=query.type
			}
			this.search()
		},
		methods:{
			...
			search(valdata=[]){
				...
				this.$api.getCWOrderDetailList_MMJ({
					"pageSize": size,      //页面大小
			...
			cancel(row){
				if(row.ifread==1){
					this.$api.doCWBackRecord({orderno:row.orderno}).then(res=>{
						if(res=='1'){
							this.$message.success('操作成功!')
							this.tableData.forEach((item,index)=>{
								if(item.orderno==row.orderno){
									this.tableData[index].ifread=1
								}
							})
						}else{
